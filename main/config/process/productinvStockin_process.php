<?php
	...
	if(ISSET($_POST)){
		...
		$add = $GYM->add_productInventory($product, $quantity, $description, $expiration_date, $price, $brand, $date_ofpurchased, $status, $select_type);
       if($add == TRUE){
		      echo '<div class="example-alert"><div class="alert alert-success alert-icon"><em class="icon ni ni-check-circle"></em> <strong>Added Product Inventory Stock In Successfully!</strong></div></div><script> setTimeout(function() {  location.replace("main/product_inventory.php"); }, 1000); </script>';
		    

		    }else{
		    echo '<div class="alert alert-danger alert-icon"><em class="icon ni ni-cross-circle"></em> <strong>Added Product Inventory Stock In failed</strong></div><script> setTimeout(function() {  location.replace("main/product_inventory.php"); }, 1000); </script>';
