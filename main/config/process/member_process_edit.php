<?php
	...
	if(ISSET($_POST)){
		...
		$edit = $GYM->edit_member($name, $age, $sex, $birthday, $plan, $email_address, $contact_no, $address, $joining_date, $expiring_date, $member_id);
       if($edit == TRUE){
		      echo '<div class="example-alert"><div class="alert alert-success alert-icon"><em class="icon ni ni-check-circle"></em> <strong>Updated Member Successfully!</strong></div></div><script> setTimeout(function() {  location.replace("main/view_member.php?id="'.$encryptedMemID.'); }, 1000); </script>';
		    

		    }else{
		    echo '<div class="alert alert-danger alert-icon"><em class="icon ni ni-cross-circle"></em> <strong>Updated Member failed</strong></div><script> setTimeout(function() {  location.replace("main/view_member.php?id="'.$encryptedMemID.'); }, 1000); </script>';
