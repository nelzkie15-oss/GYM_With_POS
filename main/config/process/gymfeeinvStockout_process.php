<?php
	...
	if(ISSET($_POST)){
		...
		$add = $GYM->add_gymfeeInventory2($product, $quantity, $description, $expiration_date, $price, $brand, $date_ofpurchased, $reason, $status, $select_type);
       if($add == TRUE){
		      echo '<div class="example-alert"><div class="alert alert-success alert-icon"><em class="icon ni ni-check-circle"></em> <strong>Added Gym Fee Inventory Stock Out Successfully!</strong></div></div><script> setTimeout(function() {  location.replace("main/gymfee_inventory.php"); }, 1000); </script>';
		    

		    }else{
		    echo '<div class="alert alert-danger alert-icon"><em class="icon ni ni-cross-circle"></em> <strong>Added Gym Fee Inventory Stock Out failed</strong></div><script> setTimeout(function() {  location.replace("main/gymfee_inventory.php"); }, 1000); </script>';
